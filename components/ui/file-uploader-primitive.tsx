...
const FileUploader = React.forwardRef<HTMLDivElement, FileUploaderProps>(
  ({ value: valueProp, onValueChange, onUpload, progresses, opts, children, className, ...props }, ref) => {
    const {
      accept = { "image/*": [] },
      maxSize = 1024 * 1024 * 4,
      maxFiles = 1,
    ...
    return (
      ...
      >
        <div ref={ref} className={cn("relative flex flex-col gap-6 overflow-hidden", className)} {...props}>
          {children}
        </div>
      </FileUploaderContext.Provider>
    );
...
const FileUploaderContent = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
...
const FileUploaderTrigger = React.forwardRef<
  HTMLDivElement,
  React.HTMLAttributes<HTMLDivElement> & VariantProps<typeof fileUploaderInputVariants>
>(({ children, className, variant, ...props }, ref) => {
  const { getRootProps, getInputProps, isDragActive, isDragAccept, isDragReject, maxFiles, maxSize, disabled } =
    useFileUploader();

  return (
    <div
      ref={ref}
      data-state={isDragActive ? "active" : isDragAccept ? "accept" : isDragReject ? "reject" : undefined}
      ...
      {isDragActive ? (
        <div className="flex flex-col items-center justify-center gap-4 sm:px-5">
          <div className="rounded-full border border-dashed p-3">
            <HiUpload className="size-7 text-muted-foreground" aria-hidden="true" />
          </div>
...
const FileUploaderItem = React.forwardRef<HTMLDivElement, FileUploaderItemProps>(
  ({ file, index, progress, className, ...props }, ref) => {
    ...
    return (
      <div ref={ref} className={cn("relative flex items-center space-x-4", className)} {...props}>
        ...
        <div className="flex items-center gap-2">
          <Button type="button" variant="outline" size="icon" className="size-7" onClick={() => onRemove(index)}>
            <RxCross2 className="size-4 " aria-hidden="true" />
